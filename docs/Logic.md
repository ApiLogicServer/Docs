...
| [Commit Constraint](Logic-Type-Constraint-Commit.md) | Boolean function must be True, checked once *after* the transaction's rule cascade has settled - not inline, per-row, like Constraint<br>else transaction rolled back | ```Rule.commit_constraint(validate=Order,```<br>```  as_condition=lambda row: row.item_count > 0,```<br>```  error_msg="Order must have at least one item")``` | Use for min-cardinality rules (e.g., *Order must have Items*) that Constraint cannot express - a Constraint on a parent can be checked before same-transaction children are added, giving a false failure |
