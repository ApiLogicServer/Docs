...
1. Read Eval-logic_bank_api.md COMPLETELY (silently - internalize full rule API)
...
2. **Read subsystem conventions first** — load `Eval-implement_requirements.md` before designing the schema:
...
3. **Check for Request Pattern signals** — apply ONLY for integration side-effects: AI calls, email, Kafka, external APIs. (`Eval-RequestObjectPattern.md`)
...
7. **Add seed data** — use `database/test_data/alp_init.py` (Flask context + LogicBank active → all computed fields auto-populated on insert). See `Eval-implement_requirements.md` Part 5 for the canonical pattern and common failure/fix pairs. Do **not** run seed scripts outside Flask context (`APILOGICPROJECT_NO_FLASK=1`) — LogicBank is suppressed and all derived fields will be zero.
...
8. **Add logic** — declare `Rule.*` rules in `logic/logic_discovery/` using `Eval-logic_bank_api.md`. Use `Rule.formula`, `Rule.sum`, `Rule.copy`, `Rule.constraint` — never procedural code in endpoints.
...
1. **`Eval-logic_bank_patterns.md`** - Foundation patterns for ALL rule types
...
2. **`Eval-logic_bank_api.md`** - Business Rules API
...
3. **`Eval-probabilistic_logic.md`** - AI Rules API
...
4. **`Eval-RequestObjectPattern.md`** - Integration services pattern
